 ...
 */
class Adminolf extends ActiveRecord implements IdentityInterface
{
    ...
    public function scenarios()     //设置多个验证场景
    {
        ...
        $parent['login'] = ['user_name','password'];
        $parent['reger'] = ['user_name','password'];
        return $parent;
    }
    ...
    {
        //获取用户名
        $rows = Admin::findOne(['user_name' => $this->user_name, 'status' => self::STATUS_ACTIVE]);
        if($rows !=null)
        {
    ...
    {
        if ($this->_user === null) {
            $this->_user = static::findOne(['user_name' => $this->user_name, 'status' => self::STATUS_ACTIVE]);
        }
        return $this->_user;
    }
    public static function findIdentity($id)
    {
        //return Admin::findOne('user_id'=>$id);
        return Admin::findOne(['user_id' => $id, 'status' => self::STATUS_ACTIVE]);
    }
