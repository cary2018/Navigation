...
{
    /**
     * @param $id this save data (传入id取得登录用户信息)
     * @param $sename this session name (传入要保存的session名称)
     * @return null|static
     */
    public static function yiisession($id,$sename)
    {
        ...
        if($id)
        {
            $make = \app\models\Admin::findOne(['id'=>$id]);
        }
        else
        {
            $make = '';
    ...
    {
        $arr = \app\models\Category::find()->all();
        $data = array();
        foreach($arr as $key)
        {
            if($show > 0)
